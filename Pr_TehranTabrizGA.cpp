...
int main(int argc, char** argv)
{
	...
	for(iters = 0; iters < num_of_iters; iters++)
	{
		...
		for(i = 0; i < num_of_trains; i++)
			if(starts[i+1] < starts[i] + 30)
				starts[i+1] = starts[i] + 30;

		starts[num_of_trains] = 10 + rand() % 10;
	...
	printf("\n\n====================================================\n");
	printf("\nThe best time-table (departure-time for trains): ");
	for(i = 0; i < 2*num_of_trains; i++)
		printf(" %d ", best_sts[i]);
	printf("\n\n");
//	fclose(report);
...
//		fprintf(fp,"P=?[F \"fin\"];\n");
		...
		float res;
		char comm1[200] = "./prism rl02.pm prop.pctl -prop 1 -sim -simwidth 29 > out02.txt";
		char comm2[200] = "./prism rl02.pm prop.pctl -prop 1 -sim -simwidth 16.7 > out02.txt";
		char comm3[200] = "./prism rl02.pm prop.pctl -prop 1 -sim -simwidth 4 > out02.txt";
		char comm4[200] = "./prism rl02.pm prop.pctl -prop 1 -sim -simwidth 1.63  > out02.txt";

		system(comm1);

		char comm_[50] = "out02.txt";
